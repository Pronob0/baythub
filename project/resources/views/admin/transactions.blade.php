          ...
          <form action="" class="d-flex flex-wrap justify-content-end">
            <div class="form-group m-1 flex-grow-1">
              <select name="" class="form-control" onChange="window.location.href=this.value">
                ...
                <option value="{{filter('remark','subscription')}}" {{request('remark')=='subscription' ? 'selected' :''}}>
                  @lang('Subscription')</option>
              ...
              <tr>
                ...
                <td data-label="@lang('Amount')">
                  <span class="{{$item->type == '+' ? 'text-success':'text-danger'}}">{{$item->type}}
                    {{$item->amount}} {{$item->currency->code}}</span>
                </td>
                <td data-label="@lang('Charge')">
                  {{$item->charge,$item->currency->type,2}} {{$item->currency->code}}
                </td>
